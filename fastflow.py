...
class FastFlow(nn.Module):
    def __init__(
        ...
        if backbone_name in [const.BACKBONE_CAIT, const.BACKBONE_DEIT]:
            self.feature_extractor =  deit.deit_base_distilled_patch16_384()
            print('Deit as feature extractor created')
            self.feature_extractor.load_state_dict(torch.load( const.FEATURE_EXTRACTOR_CHECKPOINT_PATH), strict=False)
